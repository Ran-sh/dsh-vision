/**
 * Conversation image preview enhancer. The web shell renders user messages as
 * plain text (no markdown pipeline), so the image-mind reference the send
 * hook splices (`![图片](/image-mind/raw/sha256:…)`) sits in the transcript
 * as raw text. This module watches the chat transcript — the official
 * `conversation.session` slot wrapper, which excludes the composer — and
 * upgrades each reference in place into an inline thumbnail (a real button:
 * Enter/Space opens a full-size overlay, focus returns on close). The message
 * text itself is never edited — the original markdown is restored when the
 * toggle turns off or the plugin unloads — so the session log and the model
 * side are untouched.
 *
 ...
 * full-page walks during streaming. Styles are injected as one fixed
 * <style data-plugin="image-mind"> tag (no CSS-modules pipeline needed).
 * @module dsh-plugin-image-mind/client/preview
 */

/** Matches one image-mind reference inside message text (global flag for repeated matches). */
const REFERENCE_PATTERN = /!\[([^\]]*)]\((\/image-mind\/raw\/[^)\s]+)\)/g
...
const PREVIEW_ATTR = 'data-dsh-image-mind-preview'

/** Attribute marking the full-size overlay. */
const LIGHTBOX_ATTR = 'data-dsh-image-mind-lightbox'
...
/** Locate every image-mind reference in one text chunk. Pure string math. */
...
/** The stylesheet injected on first install; fixed class names, no build-time CSS pipeline. */
const PREVIEW_CSS = [
  '.image-mind-preview { display: inline-flex; vertical-align: middle; max-width: 100%; margin: 2px 0; }',
  '.image-mind-preview > button { all: unset; cursor: zoom-in; display: inline-flex; border-radius: 8px; overflow: hidden; border: 1px solid rgba(128,128,128,.35); }',
  '.image-mind-preview > button:focus-visible { outline: 2px solid #4c9ffe; outline-offset: 1px; }',
  '.image-mind-preview img { display: block; max-width: 200px; max-height: 140px; object-fit: contain; }',
  '.image-mind-lightbox { position: fixed; inset: 0; z-index: 9999; display: flex; align-items: center; justify-content: center; background: rgba(0,0,0,.75); cursor: zoom-out; }',
  '.image-mind-lightbox img { max-width: 92vw; max-height: 92vh; object-fit: contain; border-radius: 4px; }',
].join('\n')

/** Inject the stylesheet once per document; idempotent under re-evaluation. */
function ensureStylesheet(): void {
  if (document.querySelector('style[data-plugin="image-mind"]') !== null) return
  const tag = document.createElement('style')
  tag.setAttribute('data-plugin', 'image-mind')
  tag.textContent = PREVIEW_CSS
...
export function installConversationImagePreview(isEnabled: () => boolean, root?: HTMLElement): ConversationImagePreview {
  ...
  /** Whether the text node sits inside an editable surface or our own UI. */
  ...
  const openLightbox = (src: string, alt: string, trigger: HTMLElement): void => {
    ...
    overlay.className = 'image-mind-lightbox'
    overlay.setAttribute(LIGHTBOX_ATTR, '')
    overlay.setAttribute('role', 'dialog')
    overlay.setAttribute('aria-modal', 'true')
    overlay.setAttribute('aria-label', '关闭预览')
    overlay.tabIndex = -1
  ...
  const buildPreview = (match: ImageReferenceMatch, source: string): HTMLElement => {
    ...
    preview.className = 'image-mind-preview'
  ...
  const scanNode = (node: Node): void => {
    if (node.nodeType === Node.TEXT_NODE) {
      ...
      if (text.data.includes('/image-mind/raw/') && !isExcluded(text)) enhanceNode(text)
      return
    }
    ...
    const walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, {
      acceptNode: candidate => {
        ...
        if (!text.data.includes('/image-mind/raw/')) return NodeFilter.FILTER_REJECT
