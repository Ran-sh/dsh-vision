...
|---|---|---|---|---|---|---|
| 0.3.3 | 0.3.2 | 0.1.2-rc.1 | exact `0.1.2-rc.1` family / cordis 4.0.2 / react 18.2.0 | CANDIDATE — CODE-GO (isolated lifecycle + full deterministic suite + CI 9/9); NOT PUBLISHED (registry install evidence below is the HISTORICAL 0.3.1+0.3.2 pair, not this candidate) | 2026-09-03 | Current single-track target: latest Harness `0.1.2-rc.1` (`next` tag). R2C candidate pair: `@ran-sh/dsh-vision@0.3.2` and `dsh-plugin-image-mind@0.3.3` (not yet published); plugin depends on `@ran-sh/dsh-vision@^0.3.2` because image-mind calls the circuit-breaker `admission()` API added in vision 0.3.2 — the published 0.3.1 service does NOT carry that API and must never satisfy the declared range. Earlier published combo 0.3.1 + 0.3.2 (plugin ^0.3.1) is superseded by this R2C pair. The rc.1 browser-lifecycle fixes (settings card follows the scope mirror; rc.1 positional credential transport) are in both packages. Verified in a real Chrome session on the exact rc.1 web: image-mind card renders in 设置 → 插件 → 插件配置 with the full provider UI, add-provider catalog opens, adding a provider and saving persists to `settings.yaml`, no readonly banner. Registry-only install proven for the HISTORICAL published 0.3.1+0.3.2 pair (official add/dump/boot/routes 200/remove clean, no overrides); the 0.3.2+0.3.3 candidate awaits RG-3 registry clean-room after publication. Migrates Host settings to `ctx.settings.installSection`, Client settings to `ctx.settingsScope` + `remote.credentials`, removes `dsh-client-runtime`, uses the split client packages, and binds `/image-mind` route ownership to the webServer disposer. Hosted-provider quality and the remaining browser-journey items (drag/paste, multi-image gate, session restart) remain PARTIAL (not implied). |
