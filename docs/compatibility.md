...
|---|---|---|---|---|---|---|
| 0.3.1 | 0.3.1 | 0.1.2-rc.1 | exact `0.1.2-rc.1` family / cordis 4.0.2 / react 18.2.0 | KNOWN_GOOD (isolated lifecycle + registry install + rc.1 browser settings card) | 2026-09-03 | Current single-track target: latest Harness `0.1.2-rc.1` (`next` tag). `0.3.1` published to npm (service first, then plugin; `latest` tag) with the rc.1 browser-lifecycle fixes (settings card follows the scope mirror; rc.1 positional credential transport). Verified in a real Chrome session on the exact rc.1 web: image-mind card renders in 设置 → 插件 → 插件配置 with the full provider UI, add-provider catalog opens, adding a provider and saving persists to `settings.yaml`, no readonly banner. Registry-only install proven earlier on 0.3.0 (official add/dump/boot/routes 200/remove clean, no overrides). Migrates Host settings to `ctx.settings.installSection`, Client settings to `ctx.settingsScope` + `remote.credentials`, removes `dsh-client-runtime`, uses the split client packages, and binds `/image-mind` route ownership to the webServer disposer. Hosted-provider quality and the remaining browser-journey items (drag/paste, multi-image gate, session restart) remain PARTIAL (not implied). |
