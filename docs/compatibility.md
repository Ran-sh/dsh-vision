...
|---|---|---|---|---|---|---|
| 0.3.0-alpha.1 | 0.3.0-alpha.1 | 0.1.2-alpha.5 | exact `0.1.2-alpha.5` family / cordis 4.0.2 / react 18.2.0 | PARTIAL — adaptation candidate; source/package acceptance earned, isolated local-packed lifecycle required | 2026-09-03 | Alpha-track adaptation only; does not replace the 0.2.1 / DSH 0.1.1-rc.2 KNOWN_GOOD line. Migrates Host settings registration to `ctx.settings.installSection`, Client settings to `ctx.settingsScope` + `remote.credentials`, removes `dsh-client-runtime`, uses the split alpha client packages (`api-remotes`, `api-session-controller`, `client-store`, `client-ui-settings`), and binds `/image-mind` route ownership to the webServer disposer. Registry install, hosted-provider quality and full alpha browser acceptance are not implied. |
|---|---|---|---|---|---|---|
