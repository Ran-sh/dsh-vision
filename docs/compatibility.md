...
|---|---|---|---|---|---|---|
| 0.3.0 | 0.3.0 | 0.1.2-rc.1 | exact `0.1.2-rc.1` family / cordis 4.0.2 / react 18.2.0 | KNOWN_GOOD (isolated lifecycle + registry install) | 2026-09-03 | Current single-track target: latest Harness `0.1.2-rc.1` (`next` tag). `0.3.0` published to npm (service first, then plugin; `latest` tag). Proven in a disposable DSH_HOME on exact 0.1.2-rc.1 in two stages: (1) local-packed with a pnpm-workspace overrides seam while unpublished — official `plugin --profile web add <tgz>`, `--dump-config` composes `vision-runtime` + `image-mind`, web boots (GET / 401 token gate = alive), `/image-mind/catalog`/`config`/`previews` 200, official `remove` prunes both layers, reboot 404; (2) after publication, the same gate passes with a pure registry install (`dsh-plugin-image-mind@0.3.0`, no tgz/workspace/overrides), including `npm ls`-verified 0.3.0/0.3.0 and a clean official remove. Migrates Host settings to `ctx.settings.installSection`, Client settings to `ctx.settingsScope` + `remote.credentials`, removes `dsh-client-runtime`, uses the split client packages, and binds `/image-mind` route ownership to the webServer disposer. Hosted-provider quality and full browser-journey acceptance remain PARTIAL (not implied). |
