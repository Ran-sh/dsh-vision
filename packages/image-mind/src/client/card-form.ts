...
export interface CardShell {
  ...
  failed: boolean
  /** The rejection message the Host returned for the last failed save. */
  failedReason?: string
}
...
/** An enumerated string field; only the listed choices are accepted. */
...
export class CardForm<T> {
  ...
  /**
   * Write every staged edit, then re-seed from what the Host accepted. Each
   * write is judged on read-back: a redacted secret is a write-only control
   * and lands when its set resolves. A landed field's draft is dropped; a
   * failed one stays staged for the user.
   */
  private async save(): Promise<void> {
    const plan = this.plan()
    const valid = plan.filter((item): item is { field: string; run: () => Promise<boolean> } => item.run !== undefined)
...
return plan
  ...
  private async store(field: string, value: unknown): Promise<boolean> {
    ...
    // by value would misreport a successful secret save as failed. The write
    // resolved through the scope for a secret, so it is landed.
