...
function mount(config: Config): { ctx: Context; vision: VisionRuntime; executeTool: (args: Record<string, unknown>) => Promise<unknown> } {
...
describe('composition integration (apply + VisionRuntime)', () => {
  ...
  it('D: change active A → B → the next call uses B', async () => {
    ...
    const { vision: visionB } = mount(twoProviderConfig('b'))
  ...
  it('E: a catalog provider configured → no DUPLICATE_PROVIDER, directory has one copy', () => {
    const { vision } = mount(catalogAndCustomConfig())
  ...
  it('F: catalog + custom provider → directory has catalog + custom, custom is live', () => {
    const { vision } = mount(catalogAndCustomConfig())
  ...
  it('G/H: delete all providers → routes empty; add again → routes recover', () => {
  ...
  it('J: discover explicit B while active A → B connection facts', async () => {
